...
const DailyForecast = ({ day, index, unitsSystem }) => {
  return (
    <DayContainer>
      <DateContainer>
      ...
      <DegreeView>
        <Degree style={{textAlign: 'center', fontSize: 18, fontWeight: 'bold'}}>{Math.round(day.temp.max)}°{unitsSystem}</Degree>
        <FeelsLike>Ressentie {Math.round(day.feels_like.day)}°{unitsSystem}</FeelsLike>
      </DegreeView>
    </DayContainer>
  );
};
